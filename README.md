...
1. Open the `dailyActivity_merged.csv` file. Apply a filter to all columns and check for blank values. 
2. Repeat step 1 for the `sleepDay_merged.csv` file and the `weightLogInfo_merged.csv` file. 
...
5. Now, we can compare these two metrics in a scatterplot to see if a relationship exists.
   ...
   As seen in the graph, there does not appear to be much of a relationship between the two variables with most of the values centralized in the locations mentioned prior. Calculation of the correlation coefficient between these two variables results in a small negative value (-0.06929) indicating that when total minutes asleep increases, there may be small decrease in total active minutes. This is likely due to the fact that when you spend more time asleep, you have less time to be active throughout the course of the day.
